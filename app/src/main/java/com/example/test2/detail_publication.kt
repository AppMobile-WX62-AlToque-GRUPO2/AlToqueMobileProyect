...
import com.example.test2.adapter.EspecialistAdapter
import com.example.test2.adapter.OnEspecialistClickListener
...
import com.example.test2.networking.AvailableDateService
import com.example.test2.networking.ProfessionService
import com.example.test2.networking.SpecialistService
import com.example.test2.networking.UserService
...
class detail_publication : AppCompatActivity(), OnEspecialistClickListener {
    lateinit var professionalAdapter: EspecialistAdapter
    ...
    private fun loadProfessionals() {
        ...
        professionalAdapter = EspecialistAdapter(professionals, this@detail_publication)
        rvProfessionals.adapter = professionalAdapter
    }

    override fun onAcceptClicked(especialist: User) {
        // Implementa la lógica para aceptar al especialista aquí
    ...
    override fun onDeleteClicked(especialist: User) {
        // Implementa la lógica para eliminar al especialista aquí
        Log.d("DetailPublication", "Especialist deleted: ${especialist.firstName} ${especialist.lastName}")
    }

    override fun onEspecialistClicked(especialist: User) {
        // Implementa la lógica para abrir la nueva actividad con los detalles del especialista
        val profession = userProfessionMap[especialist.id] ?: "N/A"
        val workExperience = specialistWorkExperienceMap[especialist.id] ?: 0f
        val intent = Intent(this, specialist_detail::class.java).apply {
